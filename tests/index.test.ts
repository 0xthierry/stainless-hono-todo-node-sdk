...
describe('instantiate client', () => {
  ...
  describe('defaultQuery', () => {
    ...
    test('overriding with `undefined`', () => {
      const client = new TodoHonoSDK({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' } });
  ...
  test('explicit global fetch', async () => {
    ...
    const client = new TodoHonoSDK({ baseURL: 'http://localhost:5000/', fetch: defaultFetch });
  });
  ...
  test('normalized method', async () => {
    ...
    const client = new TodoHonoSDK({ baseURL: 'http://localhost:5000/', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new TodoHonoSDK({ baseURL: 'http://localhost:5000/custom/path/' });
    ...
    afterEach(() => {
      process.env['TODO_HONO_SDK_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new TodoHonoSDK({ baseURL: 'https://example.com' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['TODO_HONO_SDK_BASE_URL'] = 'https://example.com/from_env';
      const client = new TodoHonoSDK({});
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['TODO_HONO_SDK_BASE_URL'] = ''; // empty
      const client = new TodoHonoSDK({});
      expect(client.baseURL).toEqual('https://localhost:8080/test-api');
    });

    test('blank env variable', () => {
      process.env['TODO_HONO_SDK_BASE_URL'] = '  '; // blank
      const client = new TodoHonoSDK({});
      expect(client.baseURL).toEqual('https://localhost:8080/test-api');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new TodoHonoSDK({ maxRetries: 4 });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new TodoHonoSDK({});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new TodoHonoSDK({ timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new TodoHonoSDK({ fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new TodoHonoSDK({ fetch: testFetch });
